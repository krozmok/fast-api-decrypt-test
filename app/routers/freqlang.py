...
async def create_freqlang(frequencytable:FrequencyTableIn_Pydantic):
    ...
    Args:
        frequencytable (FrequencyTableIn_Pydantic): Datos en formato JSON de la forma { "freqlang": "string" }
...
async def returns_freqlang_pk(pk:int):
    ...
    Args:
        pk (int): Entero que es el id del objeto que se busca en la base de datos de

    Returns:
        QuerySet: Resultado que contiene la información solicitada con el put request
    """
...
async def get_freqlang():
    ...
    Returns:
        QuerySet: Resultado con toda la información obtenida de la tabla FrequencyTable
    """
    return await FrequencyTable.all()

@router.get("/freqlang/{pk}")
async def get_freqlang(pk:int):
    """Retorna el resultado que es igual a la variable pk solicitada con el get request request

    Args:
        pk (int): Entro que es el iddel objeto quese busca en la base de datos

    Returns:
        QuerySet: Resultado que contiene la información solicitada con el get request request
    """
    return await FrequencyTable.filter(id=pk).first()
...
async def delete_freqlang(pk:int):
    ...
    """
    return await FrequencyTable.filter(id=pk).delete()
...
async def decrypt_message(message: Message):
    ...
    decrypted_message = Message(message=decrypt(freqlang, encrypted_message))
